						...
						if($key == 'aembarcar' || $key == 'embarcados')
						{
							...
							{	
								...
								foreach($_RETURN['embarcados']['row']['NOTAS'] as $key => $value)
								{
									...
									$nomeTransp = $value['NOM_TRANSP'];
									if($value['DT_ENT'] != ''){
										
										$newKey = 'entregue';
										$totalLabel = 'TOTAL_ENTREGUE';
										
									}else{
										
										$newKey = 'aEntregar';
										$totalLabel = 'TOTAL_AENTREGAR';
									...
									$_RETURN[$newKey]['row'][$nomeTransp]['VALOR'] += $value['VALOR'];
									$_RETURN[$newKey]['row'][$nomeTransp][$totalLabel] = count($_RETURN[$newKey]['row'][$nomeTransp]['NOTAS']);
